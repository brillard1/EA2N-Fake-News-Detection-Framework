...
class STOG(Model):
    ...
    def beam_search_with_pointer_generator(
        ...
        def beam_select_1d(input, indices):
            ...
            #print("index_value",((torch.arange(indices_size[0]).unsqueeze(1).expand_as(indices).type_as(indices) * indices_size[1] + indices).view(-1)).view(-1), input_size, indices_size)
            return input.index_select(0,
                (
                    torch.arange(
                        indices_size[0]
                    ).unsqueeze(1).expand_as(indices).type_as(indices) * indices_size[1] + indices
    ...
    def from_params(cls, vocab, params):
        ...
        bert_encoder = None
        params['bert']['pretrained_model_dir'] = 'bert-base-cased'
        params['mimick_test']['word_splitter'] = 'bert-base-cased/bert-base-cased-vocab.txt'
        print(params)
