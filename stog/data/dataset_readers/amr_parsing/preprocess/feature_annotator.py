...
def read_large_file(file_handler, block_size=3000):
    block = []
...
class FeatureAnnotator:
    ...
    def annotate(self, text):
        #print(self.nlp.annotate(text.strip(), self.nlp_properties)['sentences'])
        tokens = ast.literal_eval(self.nlp.annotate(text.strip(), self.nlp_properties))['sentences'][0]['tokens']
        output = dict(
...
if __name__ == '__main__':
    ...
    annotator = FeatureAnnotator('http://localhost:8000', args.compound_file)
