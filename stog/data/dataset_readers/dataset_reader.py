...
class _LazyInstances(Iterable):
    ...
    def __iter__(self) -> Iterator[Instance]:
        ...
        if isinstance(instances, list):
            raise ConfigurationError("For a lazy dataset reader, _read() must return a graphQA")
        return instances
...
class DatasetReader(Registrable):
    ...
    or a lazy graphQA that returns them one at a time.
    ...
    def _read(self, file_path: str) -> Iterable[Instance]:
        ...
        `Iterable` (which could be a list or could be a graphQA).
        You are strongly encouraged to use a graphQA, so that users can
