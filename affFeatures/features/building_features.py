...
from models.features.emotional.loading_emotional_lexicons import emotional_lexicons
from models.features.sentiment.loading_sentiment_lexicons import sentiment_lexicons
from models.features.morality.morality import MORALITY_class
from models.features.imageability.imageability import imageability_class
from models.features.hyperbolic.hyperbolic import hyperbolic_class
...
class segmentation_text(BaseEstimator, TransformerMixin):
    ...
    def transform(self, data):
        data = Parallel(n_jobs=1, backend="multiprocessing", prefer="processes") \
            (delayed(clean_regex)(sentence, keep_dot=False, split_text=True) for sentence in data)
...
class emotional_features(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, path='', n_jobs=1, model_name='', representation='frequency', id=''):
        self.path = path
        self.n_jobs = n_jobs
    ...
    def transform(self, data):
        file_name = './processed_files/features/emotional_features_{}_{}_{}.npy'.format(self.model_name, self.representation, self.id)
        if exists(file_name):
            ...
            #     (delayed(clean_regex)(sentence, False, True) for sentence in data)
...
class sentiment_features(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, path='', n_jobs=1, model_name='', id=''):
        self.path = path
        self.n_jobs = n_jobs
        self.model_name = model_name
        self.id = id
    ...
    def transform(self, data):
        file_name = './processed_files/features/sentiment_features_{}_{}.npy'.format(self.model_name, self.id)
        if exists(file_name):
            features = np.load(file_name, allow_pickle=True).tolist()
            features = [np.array(item) for item in features]
        else:
            ...
            features = Parallel(n_jobs=self.n_jobs, backend="multiprocessing", prefer="processes")(delayed(senti.score)(sentence) for sentence in data)
            features = [np.array(item) for item in features]
            np.save(file_name, features)
        return features
...
class morality_features(BaseEstimator, TransformerMixin):
    ...
    def transform(self, data):
        file_name = './processed_files/features/morality_features_{}_{}.npy'.format(self.model_name, self.id)
        if exists(file_name):
            features = np.load(file_name, allow_pickle=True).tolist()
            features = [np.array(item) for item in features]
        else:
            ...
            features = Parallel(n_jobs=self.n_jobs, backend="multiprocessing", prefer="processes")(delayed(lex.score)(sentence) for sentence in data)
            features = [np.array(item) for item in features]
            np.save(file_name, features)
        return features
...
class imageability_features(BaseEstimator, TransformerMixin):
    ...
    def transform(self, data):
        file_name = './processed_files/features/imageability_features_{}_{}.npy'.format(self.model_name, self.id)
        if exists(file_name):
            features = np.load(file_name, allow_pickle=True).tolist()
            features = [np.array(item) for item in features]
        else:
...
class hyperbolic_features(BaseEstimator, TransformerMixin):
    ...
    def transform(self, data):
        file_name = './processed_files/features/hyperbolic_features_{}_{}.npy'.format(self.model_name, self.id)
        if exists(file_name):
            features = np.load(file_name, allow_pickle=True).tolist()
            features = [np.array(item) for item in features]
        else:
...
def manual_features(path='', n_jobs=1, model_name='', segments_number=20, emo_rep='frequency',  id='init'):
    
    manual_feats = Pipeline([
        ('FeatureUnion', FeatureUnion([
            ('1', Pipeline([
                ('emotional_features', emotional_features(path=path, n_jobs=n_jobs, model_name=model_name, representation=emo_rep, id=id)),
            ...
            ('2', Pipeline([
                ('sentiment_features', sentiment_features(path=path, n_jobs=n_jobs, model_name=model_name, id=id)),
                ('segmentation', segmentation(n_jobs=n_jobs, segments_number=segments_number)),
                ('append', append_split_3D(segments_number=segments_number, max_len=50, mode='append')),
            ])),
            ('3', Pipeline([
                ('morality_features', morality_features(path=path, n_jobs=n_jobs, model_name=model_name, id=id)),
                ('segmentation', segmentation(n_jobs=n_jobs, segments_number=segments_number)),
                ('append', append_split_3D(segments_number=segments_number, max_len=50, mode='append')),
            ])),
            ('4', Pipeline([
                ('imageability_features', imageability_features(path=path, n_jobs=n_jobs, model_name=model_name, id=id)),
                ('segmentation', segmentation(n_jobs=n_jobs, segments_number=segments_number)),
                ('append', append_split_3D(segments_number=segments_number, max_len=50, mode='append')),
            ])),
            ('5', Pipeline([
                ('hyperbolic_features', hyperbolic_features(path=path, n_jobs=n_jobs, model_name=model_name, id=id)),
                ('segmentation', segmentation(n_jobs=n_jobs, segments_number=segments_number)),
                ('append', append_split_3D(segments_number=segments_number, max_len=50, mode='append')),
            ])),
        ], n_jobs=1)),
